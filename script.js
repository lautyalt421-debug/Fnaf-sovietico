...
const SPRITE_PATH = "assets/sprites/"; // Ruta para los GIFs
const camNames = { 1: "muelle", 2: "pasillo", 3: "calderas", 4: "patio", 5: "nodo" };

const BOTS = {
    stalnoy: { pos: 1, path: [1, 2, 4, 100], active: false, gif: "stalnoy.gif" },
    prizrak: { pos: 1, path: [1, 3, 100], active: false, gif: "prizrak.gif" },
    svyaz: { pos: 1, path: [1, 5, 2, 100], active: false, gif: "svyaz.gif" }
};
...
function toggleDoor() {
    ...
    if (GAME.doorClosed) {
        ...
        document.getElementById('btn-door').classList.add('active-btn');
    } else {
        let bg = (GAME.night === 5) ? "oficina_rota.jpg" : "oficina_base.jpg";
        office.style.backgroundImage = `url('${IMG_PATH}${bg}')`;
...
function changeCam(id) {
    ...
    setTimeout(() => {
        let enemy = "";
        let hasBot = false;
        for (let b in BOTS) { if (BOTS[b].active && BOTS[b].pos === id) { enemy = "_" + b; hasBot = true; } }
        staticL.classList.remove('static-total');
        if(hasBot) staticL.classList.add('static-strong'); else staticL.classList.remove('static-strong');
...
function updatePower() {
    if(!GAME.active) return;
    let usage = 1;
    if(GAME.doorClosed) usage++; if(GAME.camOpen) usage++; if(GAME.maskOn) usage++;
...
function triggerJumpscare(botName) {
    ...
    // Usar el GIF de la carpeta sprites
    img.src = SPRITE_PATH + BOTS[botName].gif;
    ...
    }, 1500); // Un poco más de tiempo para que se vea el GIF
}
...
function tickClock() {
    ...
    if(GAME.hour === 6) {
        ...
        GAME.active = false; alert("¡6 AM! SOBREVIVISTE"); location.reload();
    }
}
...
function startGame(n) {
    ...
    window.gameIntervals = [ setInterval(tickClock, 45000), setInterval(updatePower, 1000), setInterval(moveBots, 8000 - (n * 1000)) ];
}
...
function loadMenu() {
    ...
    for (let i = 1; i <= 6; i++) {
        ...
        btn.innerText = i === 5 ? "MOLOT" : (i === 6 ? "MODO 6" : "NOCHE "+i);
